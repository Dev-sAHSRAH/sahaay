...
def setup_qa_chain():
    ...
    if not os.path.exists(index_path):
        ...
        csv_path = "dataset/combined_dataset.csv"  
        df = pd.read_csv(csv_path)

        # Create documents from the CSV fields
        documents = []
        for _, row in df.iterrows():
            ...
            Subject: {row['subject']}
            About: {row['about']}
            What You Will Learn: {row['what_you_will_learn']}
            Syllabus: {row['syllabus']}
            Skills You Will Gain: {row['skills_you_will_gain']}
            Course URL: {row['course_url']}
            """
            document = Document(page_content=content)  # Create a Document object
...
def main():
    ...
    st.title("SAHAAY: Course Recommendation Chatbot 🤖")

    # Initialize chat history
    if "messages" not in st.session_state:
        st.session_state.messages = []
        welcome_message = (
            "Hello! I'm SAHAAY, your friendly Course Recommendation Chatbot! 🤖 "
